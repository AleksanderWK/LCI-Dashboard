...
def generate_eye_tracking_datastream(data, row, current_data):
    data_row = data.loc[row]
    end_time = data_row[2]
    next_end_time = data.loc[row + 1][2]
    current_data.append(data_row)
    #print(data_row)
    loop.call_later((next_end_time - end_time) / 1000, generate_eye_tracking_datastream, data, row + 1, current_data)

def generate_skeleton_datastream(data, row, current_data):
    row_counter = row
    time = data.loc[row][4]
    init_time = time
    while True:
        ...
        current_data.append(data_row)
        row_counter += 1
    loop.call_later(time - init_time, generate_skeleton_datastream, data, row_counter, current_data)
...
loop.call_soon(generate_eye_tracking_datastream, eye_tracking, 1, current_eye_tracking_data)
loop.call_soon(generate_skeleton_datastream, skeleton, 1, current_skeleton_data)
loop.call_soon(generate_frequency_datastream, acc, 1, current_acc_data)
loop.call_soon(generate_frequency_datastream, bvp, 1, current_bvp_data)
loop.call_soon(generate_frequency_datastream, eda, 1, current_eda_data)
loop.call_soon(generate_frequency_datastream, hr, 1, current_hr_data)
